...
export function FirefoxChrome({ url, isDark, screenshotSrc, optimisticContent, width, height }: BrowserChromeProps) {
  const toolbarBg = isDark ? 'bg-[#2b2a33]' : 'bg-[#f9f9fb]'
    ...
    <div className="window-chrome" style={{ width: `${width}px` }}>
      <div className={toolbarBg}>
        ...
        <div className="flex items-center gap-2 px-3 py-2">
          <div className={`flex-1 flex items-center gap-2 px-3 py-1.5 rounded ${isDark ? 'bg-[#1c1b22]' : 'bg-white'}`}>
      ...
      <div className={isDark ? 'bg-[#1c1b22]' : 'bg-white'} style={{ height: `${height}px` }}>
        {optimisticContent ? (
          optimisticContent
        ) : screenshotSrc ? (
          <img src={screenshotSrc} className="w-full h-full object-cover object-top" />
        ) : (
          <div className={`w-full h-full ${isDark ? 'bg-[#1c1b22]' : 'bg-[#fafafa]'}`} />
        )}
      </div>
    </div>
  )
